...
function syncAllSources() {
	...
	frappe.call({
		method: "crm.lead_syncing.background_sync.trigger_manual_sync_all",
		callback: (r) => {
			if (!r.exc) {
				toast.success(__("Sync queued for all active sources"));
			}
			isSyncingAll.value = false;
		},
		error: () => {
			toast.error(__("Error syncing sources"));
			isSyncingAll.value = false;
		},
	});
}
