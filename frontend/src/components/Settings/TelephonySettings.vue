<template>
  <div class="flex h-full flex-col gap-6 p-8">
    <div class="flex justify-between">
      <div class="flex flex-col gap-1 w-9/12">
        ...
        >
          ...
          <Badge
            v-if="twilio.isDirty || exotel.isDirty || tataTele.isDirty || mediumChanged"
            :label="__('Not Saved')"
            variant="subtle"
            theme="orange"
          />
      ...
      <div class="flex item-center space-x-2 w-3/12 justify-end">
        <Button
          :loading="twilio.save.loading || exotel.save.loading || tataTele.save.loading"
          :label="__('Update')"
          variant="solid"
          @click="update"
        />
    ...
    <div
      v-if="!twilio.get.loading || !exotel.get.loading || !tataTele.get.loading"
    ...
    >
      ...
      <FormControl
        ...
        :options="[
          ...
          { label: __('Tata Tele'), value: 'Tata Tele' },
        ]"
        class="w-1/2"
      ...
      <div v-if="isManager()" class="flex flex-col justify-between gap-4">
        <span class="text-base font-semibold text-ink-gray-8">
          {{ __('Tata Teleservices') }}
        </span>
        <FieldLayout
          v-if="tataTele?.doc && tataTeleTabs"
          :tabs="tataTeleTabs"
          :data="tataTele.doc"
    ...
    <ErrorMessage :message="twilio.save.error || exotel.save.error || tataTele.save.error || error" />
  </div>
</template>
<script setup>
...
const tataTeleFields = createResource({
  url: 'crm.api.doc.get_fields',
  cache: ['fields', 'CRM Tata Tele Settings'],
  params: {
    doctype: 'CRM Tata Tele Settings',
    allow_all_fieldtypes: true,
  },
...
const tataTele = createDocumentResource({
  doctype: 'CRM Tata Tele Settings',
  name: 'CRM Tata Tele Settings',
  fields: ['*'],
  auto: true,
  setValue: {
    onSuccess: () => {
      toast.success(__('Tata Teleservices settings updated successfully'))
    },
    onError: (err) => {
      toast.error(err.message + ': ' + err.messages[0])
    },
...
const tataTeleTabs = computed(() => {
  if (!tataTeleFields.data) return []
  let _tabs = []
  let fieldsData = tataTeleFields.data
...
function validateIfDefaultMediumIsEnabled() {
  ...
  if (defaultCallingMedium.value === 'Tata Tele' && !tataTele.doc.enabled) {
    error.value = __('Tata Tele is not enabled')
    return false
  }
  return true
}
