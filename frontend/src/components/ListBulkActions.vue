...
function startAutoDialer(selections, unselectAll) {
  const leadIds = Array.from(selections)
  
  call('crm.integrations.tata_tele.auto_dialer.start_auto_dialer', {
    lead_ids: JSON.stringify(leadIds)
  }).then((result) => {
    if (result.skipped_leads > 0) {
      toast.success(__('Auto Dialer Started: {0} leads queued, {1} skipped (no phone)', [
        result.total_leads,
        result.skipped_leads
      ]))
    } else {
      toast.success(__('Auto Dialer Started: {0} leads queued', [result.total_leads]))
    }
    
    unselectAll()
    router.push({ name: 'AutoDialerQueue', query: { queue_id: result.queue_id } })
  }).catch((error) => {
    toast.error(__('Failed to start auto dialer: {0}', [error.message]))
  })
}
...
function bulkActions(selections, unselectAll) {
  ...
  if (props.doctype === 'CRM Lead') {
    ...
    actions.push({
      label: __('Auto Dialer'),
      onClick: () => startAutoDialer(selections, unselectAll),
    })
  }
