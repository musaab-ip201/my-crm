<template>
  ...
  <Dropdown v-if="groupedActions.length" :options="groupedActions">
    <Button
      data-test-id="routing-dropdown"
      :label="dropdownLabel"
      :icon="dropdownLabel ? null : 'more-horizontal'"
      :iconRight="dropdownLabel ? 'chevron-down' : null"
    />
  </Dropdown>
  <div
...
const dropdownLabel = computed(() => {
  if (groupedActions.value.length > 0) {
    let group = groupedActions.value[0].group
    if (group === 'Routing') return __('Lead Routing')
    if (typeof group === 'string') return __(group)
  }
  return null
})
