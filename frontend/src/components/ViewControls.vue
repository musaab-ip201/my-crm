...
list.value = createResource({
  ...
  params: getParams(),
  cache: [props.doctype, route.query.view, route.params.viewType, JSON.stringify(props.filters)],
  onSuccess(data) {
...
const quickFilterList = computed(() => {
  ...
  filters.forEach((filter) => {
    ...
    // For 'assigned_user', read the value from '_assign' filter instead
    let filterKey = filter.fieldname === 'assigned_user' ? '_assign' : filter.fieldname

    if (list.value.params?.filters[filterKey]) {
      let value = list.value.params.filters[filterKey]
      if (Array.isArray(value)) {
        if (
          ...
          // For assigned_user, _assign uses LIKE, so extract the value
          if (filter.fieldname === 'assigned_user' && value[0] === 'LIKE') {
...
function applyQuickFilter(filter, value) {
  ...
  // Special handling: 'assigned_user' has no data, actual assignments are in '_assign'
  if (field === 'assigned_user') {
    delete filters['assigned_user']
    if (value) {
