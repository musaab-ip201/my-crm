...
const axisCharts = [
  ...
  { label: __('Call Life-Cycle (Sunburst)'), value: 'call_lifecycle_sunburst' },
  { label: __('Talk-Time Analysis (Scatter)'), value: 'call_time_analysis' },
  { label: __('Lead Status Analytics'), value: 'lead_status_analytics' },
]
...
const donutCharts = [
  ...
  { label: __('Lead Action Breakdown'), value: 'lead_action_insights' },
]
    ...
    onSuccess: (data = {}) => {
      ...
      items.value.push({
        ...
        layout: {
          x: 0,
          y: nextY,
          // Logic for dynamic width/height
          w:
            name.includes('sunburst') || name.includes('analysis')
              ? 20 // Custom charts look better at full width
              : type === 'call_insights' || type === 'custom'
                ? 20
                : 10,
          h:
            name.includes('sunburst') || name.includes('analysis')
              ? 10 // Give them enough vertical room to breathe
              : type === 'call_insights' || type === 'custom'
                ? 10
                : 3,
