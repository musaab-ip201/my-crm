...
async function createNewLead() {
  ...
  createLead.submit(
    ...
    },
    {
      validate() {
        error.value = null
        if (!lead.doc.salutation) {
          error.value = __('Salutation is mandatory')
          return error.value
        }
        ...
        if (!lead.doc.last_name) {
          error.value = __('Last Name is mandatory')
          return error.value
        }
        ...
        if (
         ...
         if (!/^\d{10}$/.test(lead.doc.mobile_no)) {
          error.value = __('Enter a valid 10-digit mobile number')
          return error.value
        }
