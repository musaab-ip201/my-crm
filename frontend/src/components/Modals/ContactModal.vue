...
async function createContact() {
  ...
  let validatedPhone = _contact.doc.mobile_no
    .toString()
    .replace(/\D/g, '')

  // remove country code
  if (validatedPhone.startsWith('91') && validatedPhone.length > 10) {
    validatedPhone = validatedPhone.slice(2)
  }
  ...
  if (!/^[6-9]\d{9}$/.test(validatedPhone)) {
    error.value = 'Please enter a valid mobile number'
    loading.value = false
    return
  }

  // reject repeated digits like 9999999999
  if (/^(\d)\1{9}$/.test(validatedPhone)) {
    error.value = 'Please enter a valid mobile number'
    loading.value = false
    return
  }
  ...
  _contact.doc.mobile_no = validatedPhone
