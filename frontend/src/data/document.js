  ...
  if (!documentsCache[doctype][docname || '']) {
    if (docname) {
     documentsCache[doctype][docname] = createDocumentResource({
       ...
       // STEP 1: SILENCE THE TOP-LEVEL ONERROR
       onError: (err) => {
         error.value = err;
         // DO NOT ADD TOASTS HERE
       },
       ...
       setValue: {
         ...
         // STEP 2: THIS IS THE ONLY PLACE THAT SHOULD TOAST
         onError: (err) => {
           // Prevent other events from firing multiple times
           ...
           if (err.exc_type === 'MandatoryError') {
             ...
             const fields = err.messages ? err.messages.map(m => m.split(': ').pop().trim()).join(', ') : 'Required Fields';
             message = __('Mandatory field error: {0}', [fields]);
           } else {
             // Fallback to the first message available
             message = err.messages?.[0] || __('An error occurred');
           }
     
           // SHOW ONLY ONE TOAST
           toast.error(message);
           
           console.error("Single Error Handler:", err);
         },
       },
       ...resourceOverrides
     })
