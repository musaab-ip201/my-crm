<template>
  ...
  <KanbanView
    ...
    @update="(data) => viewControlsRef.updateKanbanSettings(data)"
    @loadMore="(columnName) => viewControlsRef.loadMoreKanban(columnName)"
  >
  ...
  <DealsListView
    ...
    @applyFilter="(data) => viewControlsRef.applyFilter(data)"
    @applyLikeFilter="(data) => viewControlsRef.applyLikeFilter(data)"
    @likeDoc="(data) => viewControlsRef.likeDoc(data)"
    @selectionsChanged="
      (selections) => viewControlsRef.updateSelections(selections)
    "
  />
...
const viewControlsRef = ref(null)
...
const combinedFilters = computed(() => {
  ...
  if (route.query.status_type) {
    const statusType = route.query.status_type
    let matchingStatuses = []

    if (statusType === 'Ongoing') {
      // Dashboard counts ongoing as NOT Won/Lost (includes Open + Ongoing types)
      matchingStatuses = (dealStatuses.data || [])
    ...
    if (matchingStatuses.length) {
      baseFilters.status = ['in', matchingStatuses]
    }
  }
  return baseFilters
})

// When dealStatuses loads after page mount, re-trigger ViewControls reload
// so the status_type filter gets applied correctly (fixes race condition)
watch(
  () => dealStatuses.data,
  (data) => {
    if (data?.length && route.query.status_type && viewControlsRef.value) {
      viewControlsRef.value.reload?.()
    }
  },
)
