...
def get_data(
    ...
    if view_type != "kanban":
        ...
        if doctype == "FCRM Note" and "background_color" not in rows:
            rows.append("background_color")

        if doctype == "FCRM Note" and "creation" not in rows:
...
# 	# Extract agent_or from default_filters BEFORE anything else touches filters
# 	agent_or = None
# 	if default_filters and doctype == "CRM Call Log":
# 		try:
# 			_df = frappe.parse_json(default_filters) if isinstance(default_filters, str) else dict(default_filters)
# 			_agent = _df.pop("_agent_user", None)
# 			if not _agent and _df.get("caller") and _df.get("caller") == _df.get("receiver"):
# 				_agent = _df.pop("caller")
# 				_df.pop("receiver", None)
# 			if _agent:
# 				agent_or = _agent
# 				default_filters = _agent and frappe.as_json(_df)
# 		except Exception:
# 			pass
# 	columns = frappe.parse_json(columns or "[]")
# 	kanban_fields = frappe.parse_json(kanban_fields or "[]")
...
# 	# Strip all internal/private keys — never pass to SQL
# 	for k in [k for k in list(filters.keys()) if k.startswith("_")]:
# 		filters.pop(k, None)
...
# 		# Extract agent OR filter if set
# 		agent_or = agent_or  # already set from default_filters processing
...
# 		)
# 		if agent_or:
# 			get_list_kwargs["or_filters"] = [
# 				["caller", "=", agent_or],
# 				["receiver", "=", agent_or],
# 			]

# 		data = frappe.get_list(doctype, **get_list_kwargs) or []
# 		data = parse_list_data(data, doctype)
...
# 			doctype,
# 			filters=filters,
# 			or_filters=[["caller", "=", agent_or], ["receiver", "=", agent_or]] if agent_or else None,
# 			fields=[COUNT_NAME],
# 		)[0].total_count,
# 		"row_count": len(data),
# 		"form_script": get_form_script(doctype),
