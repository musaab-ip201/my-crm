...
def get_fresh_leads(from_date, to_date, user=""):
	"""
...
def get_follow_up_leads(from_date, to_date, user=""):
    conds = ""
    params = {
        "from_date": from_date,
        "to_date": to_date,
...
@frappe.whitelist()
def get_call_insights(from_date, to_date, user=""):
    conds = ""
    params = {
        "from_date": from_date,
        "to_date": to_date,
    ...
    if user:
        # incoming calls: agent is receiver; outgoing calls: agent is caller
        conds += " AND (caller = %(user)s OR receiver = %(user)s)"
        params["user"] = user
    ...
    return {
        ...
        "data": [
            {"label": _("Total Calls"), "value": result.total_calls},
            {"label": _("Incoming Calls"), "value": result.incoming_calls},
            {"label": _("Outgoing Calls"), "value": result.outgoing_calls},

            {"label": _("Initiated"), "value": result.initiated_calls},
            {"label": _("Ringing"), "value": result.ringing_calls},
            {"label": _("In Progress"), "value": result.in_progress_calls},
            {"label": _("Completed"), "value": result.completed_calls},
            {"label": _("Failed"), "value": result.failed_calls},
            {"label": _("Busy"), "value": result.busy_calls},
            {"label": _("No Answer"), "value": result.no_answer_calls},
            {"label": _("Queued"), "value": result.queued_calls},
            {"label": _("Canceled"), "value": result.canceled_calls},
...
def get_followup_insights(from_date="", to_date="", user=""):
	"""
...
# lead status mark done


@frappe.whitelist()
@sales_user_only
def get_lead_action_insights(from_date, to_date, user=None):
    params = {
        "from_date": from_date,
        "to_date": f"{to_date} 23:59:59",
    }

    # 1. Dynamic Total Count
    # This query counts exactly how many leads exist for this specific user/period
    user_filter = ""
    ...
    # 2. Get the Action Counts
    log_user_filter = ""
    if user and user != "Administrator":
        log_user_filter = " AND assigned_user = %(user)s"

    if not frappe.db.table_exists("Lead Department Log"):
        chart_data = [{"action": "Pending/No Action", "count": total_assigned}]
        return {
            "data": chart_data,
    ...
    sql_query = f"""
        SELECT
            CASE
                WHEN last_action = 'Forward' THEN 'Mark Done'
                WHEN last_action = 'Backward' THEN 'Send Back'
                WHEN last_action = 'Reject' THEN 'Reject to Onboarding'
                WHEN last_action = 'Manager Override' THEN 'Transfer to Department'
                ELSE last_action
            END as label_action,
    ...
    """
    action_data = frappe.db.sql(sql_query, params, as_dict=True)

    # 3. Calculate the "Remaining" leads
    # This captures all leads where NO action from the list above was taken
	...
	# Calculate Mark Done Percentage
    mark_done_count = sum(
        d["count"] for d in action_data if d.label_action == "Mark Done"
    )
    mark_done_percent = 0
    if total_assigned > 0:
...
@frappe.whitelist()
@sales_user_only
def get_lead_status_analytics(from_date, to_date, user=None):
    params = {"from_date": from_date, "to_date": f"{to_date} 23:59:59"}

    conditions = ["creation BETWEEN %(from_date)s AND %(to_date)s"]

    if user and user != "Administrator":
        conditions.append("lead_owner = %(user)s")
        params["user"] = user

    where_clause = " WHERE " + " AND ".join(conditions)

    # Fetch counts from tabCRM Lead
    data = frappe.db.sql(
        f"""
