...
@frappe.whitelist()
def get_call_insights_summary(from_date=None, to_date=None, user=None):
    # Base filter
    filters = {"type": "Incoming"}

    # LOGIC: Use start_time if available, otherwise fallback to creation
    # This is better for LIVE because it handles historical empty data
    time_filter_field = "start_time"
    
    # Quick check: do we even have start_times? 
    # (Optional: removes the need to manually switch)
    has_start_time = frappe.db.count(
    "CRM Call Log",
    filters={
        "type": "Incoming",
        "start_time": ["is", "set"],
    ...
    if not has_start_time:
        time_filter_field = "creation"

    if from_date and to_date:
        filters[time_filter_field] = ["between", [from_date, to_date]]
    
    if user:
        # Check if user is email or ID to match your DB 'receiver'
    ...
    for log in logs:
        ...
        if status in stats:
            stats[status] += 1
        elif status in ["Failed", "No Answer"]:
            stats["Missed"] += 1
        elif status == "Routed":
            stats["Ringing"] += 1
