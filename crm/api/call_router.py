...
# =========================================================
# Debug Logger — writes to logs/call_router_log
# =========================================================

def _dump_payload(event, payload):
    """Write full webhook payload to logs/call_router_log for debugging."""
    try:
        log_path = "/home/ubuntu/frappe-bench/logs/call_router_log"
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        line = f"\n{'='*60}\n[{timestamp}] EVENT: {event}\n{json.dumps(payload, indent=2)}\n"
        with open(log_path, "a") as f:
            f.write(line)
    except Exception as e:
        frappe.logger().warning(f"[SMARTFLOW] _dump_payload failed: {e}")
...
def upsert_call_log(event, payload):
    ...
    if not existing_name:
        try:
            ...
            # status updates
            if event == "received":
                doc.set("status", "Call not receive by agent")
                ...
                # disconnect reason
                reason_key = payload.get("reason_key") or ""
                if reason_key:
                    reason_lower = reason_key.strip().lower()
                    if "callee" in reason_lower:
                        doc.set("disconnect_reason", "Call disconnected by seller")
                    elif "caller" in reason_lower:
                        doc.set("disconnect_reason", "Call disconnected by agent")
                    else:
                        doc.set("disconnect_reason", reason_key)
    ...
    try:
        ...
        if event == "received":
            ...
            if reason_key:
                reason_lower = reason_key.strip().lower()
                if "callee" in reason_lower:
                    updates["disconnect_reason"] = "Call disconnected by agent"
                elif "caller" in reason_lower:
                    updates["disconnect_reason"] = "Call disconnected by seller"
                else:
                    updates["disconnect_reason"] = reason_key
...
def smartflow_inbound_received():
    ...
    try:
        ...
        payload = _get_json()
        _dump_payload("received", payload)
...
def smartflow_inbound_answered():
    ...
    try:
        ...
        payload = _get_json()
        _dump_payload("answered", payload)
        frappe.logger().info(f"[SMARTFLOW INBOUND] Payload: {json.dumps(payload, indent=2)}")
...
def smartflow_inbound_completed():
    ...
    try:
        ...
        payload = _get_json()
        _dump_payload("completed", payload)
        frappe.logger().info(f"[SMARTFLOW INBOUND] Payload: {json.dumps(payload, indent=2)}")
