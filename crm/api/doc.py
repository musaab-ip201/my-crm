...
def get_data(
	...
	if default_filters:
		...
		if doctype == "CRM Call Log":
			_agent = default_filters.pop("_agent_user", None)
			if not _agent and default_filters.get("caller") and default_filters.get("caller") == default_filters.get("receiver"):
				_agent = default_filters.pop("caller")
				default_filters.pop("receiver", None)
			if _agent:
				agent_or = _agent
		filters.update(default_filters)

	# Strip any internal keys that must never reach SQL
	# but preserve known Frappe standard fields like _assign, _liked_by, etc.
	_standard_underscore_fields = {"_assign", "_liked_by", "_user_tags", "_comments"}
	for k in [k for k in list(filters.keys()) if k.startswith("_") and k not in _standard_underscore_fields]:
		filters.pop(k, None)
...
@frappe.whitelist()
def update_note_color(name: str, color: str):
	"""Update note background_color without touching the modified timestamp."""
	frappe.db.set_value("FCRM Note", name, "background_color", color, update_modified=False)
	frappe.db.commit()
	return color
