...
def get_data(
    ...
    if not isinstance(filters, dict):
        # Signal frontend: no dynamic columns
    ...
    # Get source doc
    source_doc = frappe.get_doc("Order Sync Source", source_name)
    data_type = getattr(source_doc, "source_type", None) or "cart_data"
    if data_type == "both":
        data_type = "cart_data"
    ...
    try:
        ...
        dynamic = get_dynamic_columns_data(source_name, data_type, leads)
        ...
        if dynamic and dynamic.get("status") == "success":
            ...
            # Inject external data into each row
        ...
        frappe.log_error(title="Dynamic Columns API Error", message=str(e))
        res["_dynamic_columns"] = []
