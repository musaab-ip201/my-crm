...
class CRMLead(Document):
	...
	def before_insert(self):
		"""Auto-assign lead owner based on user role"""
		if not self.lead_owner:
			self.auto_assign_lead_owner()
	...
	def auto_assign_lead_owner(self):
		"""
		Auto-assign lead owner based on creator's role:
		- If Administrator/System Manager creates lead: distribute equally among Seller Onboarding Team users
		- If Sales User creates lead: assign to themselves
		"""
		creator = frappe.session.user
		creator_roles = frappe.get_roles(creator)
		
		# Check if creator is Administrator or System Manager
		is_admin = creator == "Administrator" or "System Manager" in creator_roles
		
		if is_admin:
			# Get users from Seller Onboarding Team
			seller_onboarding_users = self.get_seller_onboarding_team_users()
			
			if seller_onboarding_users:
				# Round-robin distribution: get the user with least leads
				assigned_user = self.get_user_with_least_leads(seller_onboarding_users)
				self.lead_owner = assigned_user
			else:
				# Fallback: assign to creator if no team members found
				self.lead_owner = creator
		else:
			# Sales User creates lead: assign to themselves
			self.lead_owner = creator

	def get_seller_onboarding_team_users(self):
		"""Get all active users from Seller Onboarding Team"""
		# Find teams with "Seller Onboarding" in department name
		teams = frappe.get_all(
			"CRM Team",
			filters={
				"enabled": 1,
				"department": ["like", "%Seller Onboarding%"]
			},
			fields=["name"]
		)
		
		if not teams:
			# Try to find by team name if department doesn't match
			teams = frappe.get_all(
				"CRM Team",
				filters={
					"enabled": 1,
					"team_name": ["like", "%Seller Onboarding%"]
				},
				fields=["name"]
			)
		...
		for member in team_members:
			user_enabled = frappe.db.get_value("User", member.user, "enabled")
			if user_enabled:
				active_users.append(member.user)
		
		return active_users

	def get_user_with_least_leads(self, users):
		"""Get the user with the least number of assigned leads (round-robin)"""
		if not users:
			return None
		...
		user_lead_counts = {}
		for user in users:
			count = frappe.db.count("CRM Lead", filters={"lead_owner": user})
			user_lead_counts[user] = count
		
		# Return user with minimum leads
	...
	def default_list_data():
		...
		list_script = """
function setupList({ list, call, createToast, router }) {
	return {
		bulk_actions: [
			{
				label: 'Auto Dialer',
				onClick: async ({ selections, unselectAll }) => {
					const leadIds = Array.from(selections);
					
					try {
						const result = await call('crm.integrations.tata_tele.auto_dialer.start_auto_dialer', {
							lead_ids: JSON.stringify(leadIds)
						});
						
						if (result.skipped_leads > 0) {
							createToast({
								title: 'Auto Dialer Started',
								text: `${result.total_leads} leads queued. ${result.skipped_leads} leads skipped (no phone number).`,
								icon: 'check',
							...
							createToast({
								title: 'Auto Dialer Started',
								text: `${result.total_leads} leads queued.`,
								icon: 'check',
								iconClasses: 'text-green-500'
							});
						}
						...
						unselectAll();
						router.push({ name: 'AutoDialerQueue', query: { queue_id: result.queue_id } });
					} catch (error) {
						createToast({
							title: 'Failed to start auto dialer',
							text: error.message,
							icon: 'x',
...
"""
		...
		return {"columns": columns, "rows": rows, "list_script": list_script}
